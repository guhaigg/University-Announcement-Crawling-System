...
从本次开始，**每次推送代码后**都必须在 README 追加一条更新记录，至少包含以下四项：

1. 更新时间（精确到分钟，建议附时区）
2. 更新内容（做了什么）
3. 修改情况（影响文件/模块、兼容性说明）
4. 新增功能页面或新增交互的使用方法（可直接按步骤操作）

建议写法：在“更新日志”新增一条，按时间倒序维护。

## 更新日志

### 2026-03-14 20:18 (+08:00)

- 更新内容
  - 完成“平衡版 + 50 所重点院校官网白名单适配”三段式改造：
    - 白名单管理页联动（新增/修改/删除/同步/加载预置 50 所）
    - 后端内置 50 所重点院校适配规则（官网入口白名单 + 匹配补强）
    - 专业查询完整回路优化（专业 -> 匹配院校 -> 单校快速执行/勾选执行）
  - 专业调剂测试新增“启用 50 所白名单补强”开关，并在结果摘要展示白名单贡献量。
- 修改情况
  - 前端：`public/index.html`、`public/app.js`
    - 新增“重点院校白名单（最多 50 所）”管理面板与交互
    - 匹配院校列表支持“仅抓该校”，并展示学院信息（仅展示不勾选）
  - 后端：`server.js`
    - 新增内置 50 所院校规则库与规则命中逻辑
    - 新增接口：`POST /api/adjustment-priority-schools/bootstrap`
    - 扩展白名单记录结构（支持 `homepageUrl` / `announcementUrls`）并接入专业调剂抓取链路
  - 兼容性：保留现有接口与页面入口；新增能力默认可直接使用，不影响旧流程。
- 新增功能使用方法
  - 白名单管理（专业调剂测试页）
    1. 进入“专业调剂测试”模块，找到“重点院校白名单（最多 50 所）”
    2. 点击“加载预置 50 所”导入内置规则
    3. 可手动新增/修改院校与学院展示信息，或点击“同步自调剂导航”
  - 专业回路执行
    1. 输入专业关键词，点击“先匹配院校”
    2. 在匹配列表中勾选院校，或直接点击“仅抓该校”
    3. 点击“执行专业测试”，查看白名单补强统计与命中结果

### 2026-03-14 18:31 (+08:00)

- 更新内容
  - 合并首页“场景入口”和“推荐回路”，移除重复入口。
  - 新增“专业调剂测试”两阶段流程：`先匹配院校 -> 勾选院校 -> 执行专业测试`。
  - 修复导航 hover 可读性问题，优化左侧与主区间距。
  - 固定院校列表改为紧凑滚动区，避免页面随着院校增加无限变长。
  - 修复研招网院校库抓取链路，刷新后院校数量显著提升。
- 修改情况
  - 前端：`public/index.html`、`public/app.js`、`public/styles.css`
  - 后端：`server.js`（院校库抓取、重试与阈值逻辑）
  - 兼容性：保留原有 API 路径；专业测试接口仅新增可选参数 `selectedSchools` 与 `previewOnly` 的前端用法。
- 新增功能使用方法
  - 专业调剂测试（院校勾选执行）
    1. 打开“专业调剂测试”，输入专业关键词（可选年份、关键词）。
    2. 点击“先匹配院校”获取候选院校列表。
    3. 勾选目标院校（可“全选院校/反选院校”）。
    4. 点击“执行专业测试”开始抓取与筛选。
  - 院校库刷新（数量异常时）
    1. 在“专业调剂测试”点击“刷新研招网本地库”。
    2. 待状态提示刷新完成后，再执行“先匹配院校”。
